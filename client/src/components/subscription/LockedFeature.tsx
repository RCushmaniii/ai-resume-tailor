...
import { useFeatureAccess, FeatureType } from '../../contexts/SubscriptionContext';
...
export function LockedFeature({
  ...
  onUpgradeClick,
  className = '',
}: LockedFeatureProps): JSX.Element {
...
}: LockedFeatureInlineProps): JSX.Element {
  const { hasAccess } = useFeatureAccess(feature);
...
export function LockedCount({
  ...
  plural = 'items',
  className = '',
}: LockedCountProps): JSX.Element | null {
  const { hasAccess } = useFeatureAccess(feature);
  ...
  return (
    ...
    >
      ...
      <SparklesIcon className="w-4 h-4 text-indigo-500 ml-auto" />
    </Link>
  );
}
